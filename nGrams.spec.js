...
describe('The buildNGrams method', function(){
  ...
  it('should be able to build unigrams', function(){
    var unigrams = nGrams.buildNGrams('Hello, world.  How are you?', 1);
    expect(unigrams).to.deep.equal({ Hello: 1, world: 1, .: 1, How: 1, are: 1, you: 1});
  });

  it('should be able to build nGrams without punctuation', function(){
    var unigrams = nGrams.buildNGrams('Hello world!', 1, {includePunctuation: false});
    expect(unigrams).to.deep.equal({ Hello: 1, world: 1 });
  });

  it('should be able to build case insensitive nGrams', function(){
    var unigrams = nGrams.buildNGrams('Hello World! Hello world!', 1, {caseSensitive: false, includePunctuation: false});
    expect(unigrams).to.deep.equal({ hello: { world: 2 }})
  });

  it('should be able to build nGrams of arbitrary length', function(){
    var bigrams = nGrams.buildNGrams("How are you doing today?", 2, {includePunctuation: false});
    expect(bigrams).to.deep.equal({ How: { are: 1 }, are: { you: 1 }, you: { doing: 1 }, doing: { today: 1 }});
    var trigrams = nGrams.buildNGrams("How are you doing today?", 3, {includePunctuation: false});
    expect(trigrams).to.deep.equal({ "How are": { you: 1 }, "are you": { doing: 1 }, "you doing": { today: 1 }});
    var quadrigram = nGrams.buildNGrams("How are you doing today?", 4, {includePunctuation: false});
    expect(quadrigrams).to.deep.equal({ "How are you": { doing: 1 }, "are you doing": { today: 1 }});
    var quintigrams = nGrams.buildNGrams("How are you doing today", 5, {includePunctuation: false})
  ...
  it('should not build nGrams greater than the length of the input text', function(){
    var trigrams = nGrams.buildNGrams("Hello, world", 3, {includePunctuation: false});
...
describe('The listAllNGrams method', function(){
  it('should work for an empty set of nGrams', function(){
    var emptyGrams = nGrams.buildNGrams("", 2, {includePunctuation: false});
  ...
  it('should return a list of all nGrams, given an input set of nGrams', function(){
    var bigrams = nGrams.buildNGrams("Hello, world!  Goodbye, world!", 2, {caseSensitive: false, includePunctuation: false});
...
describe('The getNGramsByFrequency method', function(){
  ...
  it('should return all nGrams that appear the input number of times', function(){
    var bigrams = nGrams.buildNGrams("Hello world!  How are you?  Hello world!", 2, {includePunctuation: false});
